 ...
 */
final class SvrlDetections {

    /** Fallback detection code for a violated {@code sch:assert} without an id. */
    static final String CODE_FAILED_ASSERT = "failed-assert";

    /** Fallback detection code for a triggered {@code sch:report} without an id. */
    static final String CODE_SUCCESSFUL_REPORT = "successful-report";
     ...
     */
    static ICTDetectionList toDetections(final SchematronOutput svrl, final String documentName) {
