...
/**
 * The scenario's severity overrides (the {@code customLevel} elements of the scenario
 * configuration), as an immutable detection-code → severity map. Successor of the 1.x report-XSL
 * mechanism ({@code rep:custom-level()} in {@code default-report.xsl}), applied in <b>step 7</b>
 * instead of at report-rendering time.
 * <p>
 * Semantics carried over from 1.x:
 * </p>
 * <ul>
 * <li>One {@code customLevel} element carries a <b>token list</b> of detection codes — every token
 * maps to the same level.</li>
 * <li>Overrides go in <b>both directions</b>: XRechnung downgrades CEN {@code fatal} rules to
 * warning/information <i>and</i> upgrades CEN {@code warning} rules (e.g. {@code UBL-CR-646},
 * {@code CII-SR-*}) to error.</li>
 * <li>Level mapping into the severity model: {@code error} → {@link CTStandardSeverity#ERROR},
 * {@code warning} → {@link CTStandardSeverity#WARNING}, {@code information} →
 * {@link CTStandardSeverity#NONE}.</li>
 * <li>Engine/processing detections are never overridable (1.x: {@code PROCESSING_ERROR} exemption).
 * This is enforced structurally: {@code ApplyRulesAction} applies overrides only to the findings
 * produced by the rules themselves, never to its own engine-error/skip markers.</li>
 * </ul>
 * <p>
 * This class is pure data — the application (and the structural protection above) lives in
 * {@code ApplyRulesAction}.
 * </p>
 *
...
public final class SeverityOverrides
{
  ...
  /**
   * Collects all {@code customLevel} overrides declared by the given scenario configuration (across
   * all {@code createReport} elements, token lists expanded).
   *
   * @param configuration
   *        the scenario configuration; {@code null} yields {@link #NONE}
  ...
  /**
   * Convenience accessor for pipeline assembly: the overrides carried by the selected scenario
   * match.
   *
   * @param match
   *        the selected scenario from step 4; foreign implementations yield {@link #NONE}
  ...
  /**
   * The effective severity for the given detection code, or {@code null} when the scenario declares
   * no override for it (the declared severity stands).
   *
   * @param code
   *        the detection code (SVRL assert id / rule code)
