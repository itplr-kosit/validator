...
public class ApplyRulesActionTest
{
  ...
  private final ContentRepository repository = new ContentRepository (TestHelper.getTestProcessor (),
                                                                      ResolvingMode.STRICT_RELATIVE.getStrategy (),
                                                                      Simple.REPOSITORY_URI);

  private @NonNull CTParsedValidationSource parse (final @NonNull URI document) throws IOException
  {
    final ParseXmlResult parsed = new ParseXmlAction ().execute (ReadResource.of (Resource.of (document),
                                                                                  resHelper.get ()));
  ...
  {
    final RetrieveArtifactsAction.RetrieveArtifactsResult retrieved = new RetrieveArtifactsAction (Simple.REPOSITORY_URI).execute (Arrays.stream (references)
                                                                                                                                         .map (ValidationArtifactReference::of)
                                                                                                                                         .toList (),
                                                                                                                                   "test");
  ...
  public void testCleanRunAppliesAllRuleSetsInOrder () throws IOException
  {
    ...
    assertThat (result.detections ().getAll ()).extracting ("code")
                                               .containsExactly (ApplyRulesAction.CODE_RULES_APPLIED,
  ...
  {
    final ApplyRulesActionResult result = this.action.execute (parse (Simple.SCHEMATRON_INVALID),
  ...
  public void testCustomLevelOverrideDowngradesFinding () throws IOException
  {
    ...
    assertThat (result.isSuccess ()).isTrue ();
    assertThat (result.result ().hasErrors ()).isFalse ();
    final CTDetection overridden = result.detections ()
                                         .getAll ()
                                         .stream ()
                                         .filter (d -> "content-1".equals (d.getCode ()))
                                         .findFirst ()
                                         .orElseThrow ();
  ...
  public void testEngineErrorAndSkipMarkersAreNeverOverridable () throws IOException
  {
    ...
    final SeverityOverrides overrides = overridesFor (ApplyRulesAction.CODE_RULE_ENGINE_ERROR,
                                                      ErrorLevelType.INFORMATION);
    final ApplyRulesActionResult result = this.action.execute (parse (Simple.SIMPLE_VALID),
                                                               prepare ("simple-runtime-error.sch", "simple.sch"),
                                                               overrides);

    assertThat (result.status ()).isEqualTo (CTStepResult.FAILURE);
    final CTDetection engineError = result.detections ()
                                          .getAll ()
                                          .stream ()
                                          .filter (d -> ApplyRulesAction.CODE_RULE_ENGINE_ERROR.equals (d.getCode ()))
                                          .findFirst ()
                                          .orElseThrow ();
  ...
  public void testNoRuleSetsSkipsTheStep () throws IOException
  {
    ...
    assertThat (result.detections ().getAll ()).extracting ("code")
                                               .containsExactly (ApplyRulesAction.CODE_STEP_SKIPPED);
  }
}
