 ...
 * failure path for every step and require a partial CVR to be emitted, so each of those paths needs a report that a
 ...
 * </p>
 */
public class CvrUnhappyPathTest {
     ...
     * CVR is produced for them. Until artifact resolution and rule preparation belong to the pipeline alone, the only
     ...
     * before the failure stay in the report and stay valid — that is what makes a partial CVR diagnostic.
     *
     ...
     */
    private static void assertCancelledAt(final Document cvr, final CTActionType failingStep, final String expectedCode) {
        ...
        final List<Element> reports = reports(cvr);
    ...
    public void step2NotWellformed() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SCENARIOS_WITH_SCH, TestResources.Simple.NOT_WELLFORMED, null,
                "step2-not-wellformed.xml");

        assertCancelledAt(cvr, CTActionType.PARSE_DOCUMENT, XmlDetection.CODE_NOT_WELLFORMED);
    ...
    public void step3NoScenarioMatches() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SCENARIOS_WITH_SCH, TestResources.Simple.UNKNOWN, null,
                "step3-no-scenario-matched.xml");

        assertCancelledAt(cvr, CTActionType.DETECT_SCENARIOS, DetectScenariosAction.CODE_NO_SCENARIO_MATCHED);
        // without a scenario there is nothing to identify or locate
        final Element detection = (Element) reports(cvr).get(1).getElementsByTagNameNS(NS, "detection").item(0);
    ...
    public void step3RequestedScenarioIsUnknown() throws Exception {
        ...
        assertCancelledAt(cvr, CTActionType.DETECT_SCENARIOS, DetectScenariosAction.CODE_SCENARIO_UNKNOWN_ID);
    ...
    public void step4ScenarioIsAmbiguous() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SCENARIOS_AMBIGUOUS, TestResources.Simple.SIMPLE_VALID, null,
                "step4-scenario-ambiguous.xml");

        assertCancelledAt(cvr, CTActionType.SELECT_SCENARIO, SelectScenarioAction.CODE_SCENARIO_AMBIGUOUS);
        // detection succeeded and reported both candidates — the ambiguity is a selection problem, not a detection one
        assertThat(reports(cvr).get(1).getElementsByTagNameNS(NS, "detection").getLength()).isEqualTo(2);
    }

    @Test
    public void step5ArtifactIsMissing() throws Exception {
        final Document cvr = serialize(runWithReferences(TestResources.Simple.SIMPLE_VALID, "simple.xsd", "does-not-exist.sch"),
                "simple.xml", "step5-artifact-missing.xml");

        assertCancelledAt(cvr, CTActionType.RETRIEVE_ARTIFACTS, RetrieveArtifactsAction.CODE_ARTIFACT_MISSING);
        // knowing *which* artifact is missing is the whole point, so it is named and located on the detection
        final Element failing = lastDetectionWithCode(cvr, RetrieveArtifactsAction.CODE_ARTIFACT_MISSING);
    ...
    public void step6RulesDoNotCompile() throws Exception {
        final Document cvr = serialize(runWithReferences(TestResources.Simple.SIMPLE_VALID, "does-not-compile.sch"), "simple.xml",
                "step6-rule-prepare-error.xml");

        assertCancelledAt(cvr, CTActionType.PREPARE_RULES, PrepareRulesAction.CODE_RULE_PREPARE_ERROR);
        // the artifact was retrieved fine — the failure is in preparing it, and the report separates the two
        assertThat(creator(reports(cvr).get(3))).isEqualTo(CTActionType.RETRIEVE_ARTIFACTS.getName());
        assertThat(digest(reports(cvr).get(3)).getAttribute("valid")).isEqualTo("true");
    }

    @Test
    public void step7RuleEngineFails() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SCENARIOS_ENGINE_ERROR, TestResources.Simple.SIMPLE_VALID, null,
                "step7-rule-engine-error.xml");

        assertCancelledAt(cvr, CTActionType.APPLY_RULES, ApplyRulesAction.CODE_RULE_ENGINE_ERROR);
        // an engine error is not a finding: it cancels, and conformance is never computed
        assertThat(reports(cvr)).extracting(CvrUnhappyPathTest::creator).doesNotContain(CTActionType.COMPUTE_CONFORMANCE.getName());
    }

    private static Element lastDetectionWithCode(final Document cvr, final String code) {
        final NodeList detections = cvr.getElementsByTagNameNS(NS, "detection");
        Element ret = null;
