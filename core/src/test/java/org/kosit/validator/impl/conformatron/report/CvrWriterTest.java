...
/**
 * Tests the {@link CvrWriter} draft format: a completed run serializes with one report per step (and per rule set for
 * APPLY_RULES), a cancelled run still serializes as partial CVR (ADR-004 constraint 2).
 */
public class CvrWriterTest {
    ...
    private Document serialize(final URI document) throws Exception {
        ...
        // CVR is a profile of XVRL: a report that does not validate against it is not a CVR report
        CvrAssert.assertValidCvr("test-document.xml", out.toByteArray());
    ...
    private static List<Element> reports(final Document cvr) {
        final NodeList nodes = cvr.getElementsByTagNameNS(NS, "report");
    ...
    public void testTimestampAlwaysCarriesSeconds() {
        ...
        assertThat(CvrWriter.timestamp(onTheMinute)).isEqualTo("2026-08-31T15:48:00+02:00");
        assertThat(CvrWriter.timestamp(onTheMinute.withSecond(7))).isEqualTo("2026-08-31T15:48:07+02:00");
    }

    @Test
    public void testCompletedRunSerializesOneReportPerStepExecution() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SIMPLE_VALID);
        final Element root = cvr.getDocumentElement();

        assertThat(root.getLocalName()).isEqualTo("reports");
        assertThat(root.getAttributeNS(NS_CVR, "conformant")).isEqualTo("true");
        assertThat(root.getAttributeNS(NS_CVR, "status")).isEqualTo("COMPLETED");
        ...
        assertThat(document.getAttributeNS(NS_CVR, "checksum")).isEmpty();
        // the document hash is context of the detection, not one of its messages
        final Element parseReport = reports(cvr).get(0);
        final Element hash = (Element) parseReport.getElementsByTagNameNS(NS_CVR, "hash").item(0);
        assertThat(hash.getAttributeNS(NS_CVR, "algorithm")).isEqualTo("SHA-512");
        ...
        assertThat(payloadMessage.getAttributeNS(XMLConstants.XML_NS_URI, "id")).isEqualTo(CvrWriter.ID_DOCUMENT_CONTENT);
        assertThat(payloadMessage.getAttributeNS(NS_CVR, "mime-type")).isEqualTo("application/xml");
        // UTF-8 XML goes in as a DOM fragment, and then the source encoding says nothing worth writing
        assertThat(payloadMessage.getAttributeNS(NS_CVR, "encoding")).isEqualTo(CvrWriter.ENCODING_DOM);
        assertThat(payloadMessage.hasAttributeNS(NS_CVR, "source-encoding")).isFalse();
        ...
        assertThat(reports(cvr)).extracting(CvrWriterTest::creator).containsExactly(CTActionType.PARSE_DOCUMENT.getName(),
        ...
        final Element verdict = (Element) reports(cvr).get(8).getElementsByTagNameNS(NS, "detection").item(0);
        assertThat(verdict.getAttributeNS(NS_CVR, "decision")).isEqualTo("ACCEPT");
        assertThat(verdict.hasAttribute("severity")).isFalse();
        // the APPLY_RULES reports carry the rule set identity
        final Element schematronReport = reports(cvr).get(6);
        ...
        assertThat(schema.getAttribute("schematypens")).isEqualTo(CvrWriter.SCHEMATYPENS_SCHEMATRON);
        assertThat(schema.getAttributeNS(NS_CVR, "phase")).isEqualTo("#ALL");
    }

    @Test
    public void testNonUtf8SourceIsEmbeddedAsBase64() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SIMPLE_LATIN1);

        final NodeList messages = reports(cvr).get(0).getElementsByTagNameNS(NS, "message");
        final Element payloadMessage = (Element) messages.item(0);
        // transcoding into the UTF-8 report would lose the original bytes, so the source travels base64
        assertThat(payloadMessage.getAttributeNS(NS_CVR, "encoding")).isEqualTo(CvrWriter.ENCODING_BASE64);
        assertThat(payloadMessage.getAttributeNS(NS_CVR, "source-encoding")).isEqualTo("ISO-8859-1");
    ...
    public void testScenarioDetectionsCarryIdAndLocation() throws Exception {
        ...
        final Element detectReport = reports(cvr).get(1);
    ...
    public void testSelectedScenarioIsEmbeddedInFull() throws Exception {
        ...
        final Element selectReport = reports(cvr).get(2);
        ...
        assertThat(scenarioMessage.getAttributeNS(XMLConstants.XML_NS_URI, "id")).isEqualTo(CvrWriter.ID_SCENARIO_CONTENT);
        // scenario configurations are UTF-8 by definition, so the scenario is always embedded as a DOM fragment
        assertThat(scenarioMessage.getAttributeNS(NS_CVR, "encoding")).isEqualTo(CvrWriter.ENCODING_DOM);
    ...
    public void testFindingsAppearWithDigestAndCodes() throws Exception {
        final Document cvr = serialize(TestResources.Simple.SCHEMATRON_INVALID);

        assertThat(cvr.getDocumentElement().getAttributeNS(NS_CVR, "conformant")).isEqualTo("false");
        final Element schematronReport = reports(cvr).get(6);
    ...
    @Test
    public void testCancelledRunStillSerializesAsPartialCvr() throws Exception {
        final Document cvr = serialize(TestResources.Simple.NOT_WELLFORMED);
        final Element root = cvr.getDocumentElement();

        assertThat(root.getAttributeNS(NS_CVR, "status")).isEqualTo("CANCELLED");
        assertThat(root.getAttributeNS(NS_CVR, "conformant")).isEqualTo("false");
        // only the executed step is reported — plus the decision, which always runs and rejects a cancelled run
        assertThat(reports(cvr)).extracting(CvrWriterTest::creator).containsExactly("parse-document", "decision-recommendation");
        final Element verdict = (Element) reports(cvr).get(1).getElementsByTagNameNS(NS, "detection").item(0);
        ...
        final NodeList messages = cvr.getElementsByTagNameNS(NS, "message");
        for (int i = 0; i < messages.getLength(); i++) {
            assertThat(((Element) messages.item(i)).getAttributeNS(NS_CVR, "mime-type")).isEmpty();
        }
        final Element digest = (Element) cvr.getElementsByTagNameNS(NS, "digest").item(0);
        assertThat(digest.getAttribute("valid")).isEqualTo("false");
        assertThat(digest.getAttribute("error-count")).isEqualTo("1");
