...
public class SchematronValidationTest {
    ...
    public void testConformantDocument() {
        final AdHocValidationResult result = this.validation.validate(TestHelper.read(TestResources.Simple.SIMPLE_VALID),
    ...
    public void testDocumentWithFindings() {
        final AdHocValidationResult result = this.validation.validate(TestHelper.read(TestResources.Simple.SCHEMATRON_INVALID),
                UriHelper.resolve(TestResources.Simple.REPOSITORY_URI, "simple.sch", true), true);
    ...
    public void testProcessingErrorFailsTheRun() {
        final AdHocValidationResult result = this.validation.validate(TestHelper.read(TestResources.Simple.SIMPLE_VALID),
                UriHelper.resolve(TestResources.Simple.REPOSITORY_URI, "simple-runtime-error.sch", true), true);
    ...
    public void testSchematronThatHasNoRepositoryIsReportedAsDetection() {
        ...
        final AdHocValidationResult packaged = this.validation.validate(TestHelper.read(TestResources.Simple.SIMPLE_VALID),
                UriHelper.resolve(TestResources.getJarRepository(), "simple.sch", true));
        final AdHocValidationResult relative = this.validation.validate(TestHelper.read(TestResources.Simple.SIMPLE_VALID),
                URI.create("simple.sch"));
    ...
    public void testMissingSchematronFailsInRetrieveStep() {
        final AdHocValidationResult result = this.validation.validate(TestHelper.read(TestResources.Simple.SIMPLE_VALID),
                UriHelper.resolve(TestResources.Simple.REPOSITORY_URI, "does-not-exist.sch", true), true);
    ...
    public void testNotWellformedDocumentFailsBeforeRules() {
        final AdHocValidationResult result = this.validation.validate(TestHelper.read(TestResources.Simple.NOT_WELLFORMED),
                UriHelper.resolve(TestResources.Simple.REPOSITORY_URI, "simple.sch", true), true);
