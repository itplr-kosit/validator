...
public final class Scenario3Mapper {
    ...
    private static @NonNull ScenarioSchematron _toGeneric(@NonNull final ValidateWithSchematronType src) {
        final ScenarioSchematron ret = new ScenarioSchematron(_toGeneric(src.getResource())).setPsvi(src.isPsvi())
                .setCompiler(src.getCompiler());
        for (final CustomErrorLevelType customLevel : src.getCustomLevel()) {
    ...
    private static @NonNull Scenario _toGeneric(@NonNull final ScenarioXmlType src) {
        ...
        for (final ValidateWithSchematronType schematron : src.getValidateWithSchematron()) {
            ret.addSchematron(_toGeneric(schematron));
        }
    ...
    private static @NonNull ScenarioXmlType _fromGenericXml(@NonNull final Scenario src, @NonNull final ConversionState state) {
        ...
        // A scenario without XML Schema validates with Schematron alone
        ...
        for (final ScenarioSchematron schematron : src.getSchematrons()) {
            ...
            for (final ScenarioCustomErrorLevel customLevel : schematron.getCustomLevels()) {
