...
public class ScenarioMappingTest {
    ...
    public void readsVersion2IntoTheGenericModel() throws URISyntaxException {
        ...
        // The severity overrides are declared with the rule set they belong to
        assertThat(simple.getSchematrons()).singleElement().satisfies(x -> {
            assertThat(x.getResource().getLocation()).isEqualTo("simple.xsl");
    ...
    public void version3ToVersion2WritesAValidDocument() throws URISyntaxException {
        ...
        config.setFrameworkVersion("2.0.0");
        ...
        assertThat(xml).contains(Scenario2Converter.NS_URI).contains("Peppol Billing BIS 3 - UBL Invoice").doesNotContain("Factur-X basic")
    ...
    public void version2ToVersion3WritesAValidDocumentOnceTheCoordinatesAreSet() throws URISyntaxException {
        ...
        final String xml = this.converter3.writeXml(converted);
        assertThat(xml).contains(Scenario3Converter.NS_URI).contains("<groupId>org.example</groupId>").contains("<scenarioXml>")
                .doesNotContain("<scenarioPdf>");
    }

    @Test
    public void keepsTheSeverityOverridesOfARuleSetInBothVersions() throws URISyntaxException {
        final ScenarioConfiguration config = Scenario2Mapper.toGeneric(readV2());
        addCoordinates(config);

        // Version 3 declares the overrides with the rule set as well
        final String xml3 = this.converter3.writeXml(Scenario3Mapper.fromGeneric(config));
        assertThat(xml3).contains("<customLevel level=\"warning\">BR-01 BR-02</customLevel>");

        final ScenarioConfiguration roundTripped = Scenario3Mapper.toGeneric(this.converter3.readXml(xml3));
        assertThat(roundTripped.getScenarios().get(0).getSchematrons()).singleElement()
                .satisfies(x -> assertThat(x.getCustomLevels()).singleElement().satisfies(y -> {
                    assertThat(y.getLevel()).isEqualTo(EScenarioErrorLevel.WARNING);
                    assertThat(y.getRuleIDs()).containsExactly("BR-01", "BR-02");
                }));
    }
    ...
    public void convertsTheDescriptionBlockKindsIntoBothVersions() {
        ...
        config.setFrameworkVersion("2.0.0").setLastModificationDate(LocalDate.of(2026, 8, 28));
