...
- (API) `scenarios-v1.xsd`: `validateWithSchematron` takes the `customLevel` overrides of its rule set, so the severity overrides are declared with the rules they belong to instead of below `createReport`. `createReport/customLevel` is deprecated but still read; for a code named in both places the rule set decides (`ScenarioSeverityOverrides`)
