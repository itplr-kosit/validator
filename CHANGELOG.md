...
- (BUILD) Added the new submodule `cvr`, built after `xvrl` and depending on `xvrl` and `schematron`. It carries the CVRL profile - CVRL is the Conformatron Validation Result, the XVRL profile of the validator - as `xsd/cvrl.xsd` for the extension vocabulary and `sch/cvrl.sch` for the profile constraints, applied through `org.kosit.cvr.report.CvrlProfile`
- (API) `CvrlProfile.validate(String, byte[])` checks a report twice: against `xvrl-1.0.xsd` for the structure and against the profile Schematron for everything that makes an XVRL report a CVRL report - the canonical step reports and their order, the extension vocabulary, and the internal consistency of digest, detections and run status. The constraints are not in the XSD because XVRL admits foreign attributes with `processContents="skip"` and because most of them relate one part of the report to another
- (CORE) The CVRL tests validate the generated reports against the profile instead of only against the XVRL schema
- (API) `org.kosit.conformatron.detection.Detection` implements the complete `CTDetection` interface now, so `getID()`, `getField()` and `getSummary()` return the values provided instead of always `null`. Following the XVRL data model, instances are created through the static `builder()` methods (`builder()`, `builderError()`, `builderWarning()`, `builderNone()`) and can be derived from an existing instance via `toBuilder()`. The public constructor was removed; the factory methods `Detection.of(...)` and `Detection.overridden(...)` remain. `Detection.overridden(...)` keeps the detection time of the base detection and now also carries over its ID, field and summary
