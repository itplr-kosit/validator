...
<<<<<<< HEAD
=======
- (BUILD) The release archive is built by the new `distribution` module: `distribution/target/validator-<version>.zip` holds `validator-cli-<version>.jar` and `validator-server-<version>-runner.jar` at the top and everything the command line application loads below `libs/`, with LICENSE, NOTICE and the changelog beside them. The aggregator kept building an archive of its own output, which since the modularization is nothing but LICENSE and NOTICE - an aggregator is built before its modules, so the archive of the whole product has to be a module of its own, and the last one in the reactor
- (CLI) The shaded jar is attached as `validator-cli-<version>-standalone.jar` instead of replacing the main artifact, and the main artifact is the thin jar again, with a `Class-Path` pointing at `libs/`. `java -jar validator-cli-<version>.jar` therefore works straight out of the unpacked distribution, and whoever depends on `org.kosit:validator-cli` gets the module instead of an archive of the whole dependency tree
- (CORE) New test `ConformanceValidationConcurrencyTest`: one engine, sixteen threads, a mixed set of documents covering a completed run, a schema violation, a rule finding and two cancel paths. Every concurrent run has to reproduce the verdict taken single-threaded beforehand, down to the codes of all detections. It is a regression guard for the shared state the engine and its repositories hold, and it has teeth: with the previously shared `SchemaFactory` restored it fails on every run, reporting `REJECT` with `rule-prepare-error` for a document whose verdict is `ACCEPT`
- (CORE) `ContentRepository` compiles every XML Schema with its own `SchemaFactory` and caches the result. A single factory was held per repository and written on the request path (`setResourceResolver(null)` before `newSchema`), while one repository serves every request thread of a server - a `SchemaFactory` is not thread-safe by its own JAXP contract, so two concurrent schema compilations could resolve imports against each other or corrupt the grammar pool. The strategy hands out a fresh, hardened factory per compilation instead. The compiled `Schema` is immutable and thread-safe, so it is now kept per repository and URI set: schemas were previously recompiled from disk for every single document. A failed compilation is not cached. The protected `ContentRepository` constructor no longer takes a `SchemaFactory`
- (CORE) `ConformanceValidation` keeps its per-repository artifact retrieval in a `ConcurrentHashMap`. It was a plain `HashMap`, written on the run path through `computeIfAbsent`, while one engine serves all request threads of a server: concurrent writes to a `HashMap` can lose entries or damage the table during a resize. What the map holds was already safe to share, a `RetrieveArtifactsAction` carries nothing but an immutable `ArtifactResolver`. The class documentation no longer claims the engine holds no state of its own; it holds this one cache, which is derived from the scenarios and does not change with the document
>>>>>>> 3e88558e (Updating distribution of the validator)
- (API) `ProcessorProvider` builds its Saxon processor in the class initializer and holds it in a `static final` field. It was assigned to a plain, non-volatile field on the first call to `getProcessor()`, which under concurrency both allowed two processors to be built and - the reason for the change - let one thread publish the reference before the hardening applied after the constructor became visible to another. A thread could therefore work with a processor on which `ALLOW_EXTERNAL_FUNCTIONS` was still at its default. The server shares one processor across all request threads, so the window was reachable. A failure while configuring the processor now surfaces as an `ExceptionInInitializerError` on first use instead of an exception from `getProcessor()`
=======
>>>>>>> cce665f1 (cancel step 3 when a match expression can not be evaluated)
=======
- (API) `ProcessorProvider` builds its Saxon processor in the class initializer and holds it in a `static final` field. It was assigned to a plain, non-volatile field on the first call to `getProcessor()`, which under concurrency both allowed two processors to be built and - the reason for the change - let one thread publish the reference before the hardening applied after the constructor became visible to another. A thread could therefore work with a processor on which `ALLOW_EXTERNAL_FUNCTIONS` was still at its default. The server shares one processor across all request threads, so the window was reachable. A failure while configuring the processor now surfaces as an `ExceptionInInitializerError` on first use instead of an exception from `getProcessor()`
>>>>>>> cc7d0fc9 (build the Saxon processor in the class initializer)
